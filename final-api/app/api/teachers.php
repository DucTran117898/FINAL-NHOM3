...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($action === null || $action === '') {
            ...
            // Support both JSON and multipart/form-data (FormData) requests
            $contentType = $_SERVER['CONTENT_TYPE'] ?? '';

            if (strpos($contentType, 'multipart/form-data') === 0) {
                // Handle create with FormData (file upload)
                $errors = [];
                ...
                // Avatar is required when creating a new teacher
                $avatar_filename = '';
                ...
                // Handle JSON-based create (e.g. from API clients)
                if (!$input || !isset($input['name']) || $input['name'] === '') {
                    http_response_code(400);
                    echo json_encode([
                        'success' => false,
